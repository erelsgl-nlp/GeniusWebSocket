...
/**
 * Objective is a collection of related {@link Issue}s about which people negotiate.
 * 
 * @note Some work needs to be done to guarantee consistency of the tree. Methods like setParent don't signal 
...
public class Objective implements MutableTreeNode 
{
	...
	private ArrayList<Objective> childrenObjectives = new ArrayList<Objective>();
	...
	public boolean equalContents(Objective obj)
	{
		...
		for (Objective child:childrenObjectives)
	...
	public void addChild(Objective newObjective) {
		childrenObjectives.add(newObjective);
	...
	public Enumeration<Objective> children() {
		  return Collections.enumeration(childrenObjectives);
	}
	...
	{ return new ArrayList<Objective>(childrenObjectives); }
	...
	public Objective getChildAt(int childIndex) {
		if (childIndex < childrenObjectives.size() && childIndex >= 0)
			return childrenObjectives.get(childIndex);
		else
			return null;
	}
	...
	public int getChildCount() {
		return childrenObjectives.size();
	}
	...
	public int getIndex(TreeNode node) {
		for (int i = 0; i < childrenObjectives.size(); i++) {
			if (node == childrenObjectives.get(i))
				return i;
		}
		return -1;
	}
	...
	public void insert(MutableTreeNode child, int index) {
		if (index <= getChildCount() && index >= 0) {
			childrenObjectives.add(index, (Objective)child);
	...
	public void remove(int index) {
		if (index < getChildCount() && index >= 0) {
			...
			childrenObjectives.remove(index);
		}
		// TODO Update weights
	}
